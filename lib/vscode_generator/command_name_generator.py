...
class CommandNameGenerator(BaseGenerator):
    ...
    def process():
        ...
        with SystemConfig().command_names_ts.open("w", encoding="utf-8") as f:
            command_csv_file = SystemConfig().get_csv_file(ItemType.COMMAND)
            ...
            f.write(f"// - Parsed Commands:  {SystemConfig().get_csv_file(ItemType.COMMAND, rel_path=True).as_posix()}\n")
            f.write(f"// - Manual Overrides: {SystemConfig().get_patch_csv_file(ItemType.COMMAND, rel_path=True).as_posix()}\n")
            f.write(f"// - Parsed Functions: {SystemConfig().get_csv_file(ItemType.FUNCTION, rel_path=True).as_posix()}\n")
            f.write(f"// - Manual Overrides: {SystemConfig().get_patch_csv_file(ItemType.FUNCTION, rel_path=True).as_posix()}\n")
